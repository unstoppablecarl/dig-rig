...
import { DRAW_PARTICLE_DEBUG, MAX_VFX_MATTER_PARTICLES, VFX_PARTICLE_TO_TERRAIN_CHUNK_SIZE } from '../../config.ts'
...
export class VFXParticleManager extends SceneBound {
  ...
  public constructor(public scene: GameLevel) {
    ...
    this.emitter = this.scene.sys.displayList.add(new EmitterClass(this.scene, 0, 0, 'particle', {
      ...
      speed: 0,
      quantity: Math.min(1000, MAX_VFX_MATTER_PARTICLES),
      // manual
      frequency: -1,
      maxAliveParticles: MAX_VFX_MATTER_PARTICLES,
      emitting: false,
  ...
  spawnMatterToTiles(source: Position, tiles: ReadonlyArray<Position>) {
    if (MAX_VFX_MATTER_PARTICLES <= 0) return
